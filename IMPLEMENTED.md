...
| `QualityEvaluator.load()` | Method | Downloads and loads the CLIP ViT-B/32 model and processor from HuggingFace. Puts the model in eval mode on GPU. No-op if already loaded. |
| `QualityEvaluator.clip_score(prompt, image)` | Method | Computes the CLIP cosine-similarity between the text prompt and generated image, normalises it to [0, 1] via sigmoid. |
